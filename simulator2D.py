...
class TrajectoryComputer:
    def __init__(self, physics_service: PhysicsService, solver_service: BesselAnomalySolver, drag: DragForce):
        ...
        # OPTIMIZATION: Pre-calculate GM as float. Doing this once reduces overhead in the hot loop.
        # PhysicsService stores Decimals, so this converts the Decimal result to float.
        self.GM_float = float(self.physics.G * (self.physics.M + self.physics.m))

        # End states
        self.destroyed = False 
        self.escaped = False   
        self.has_collided = False  # start false; flip on collision
    ...
    def set_initial_from_orbital_elements(self, a, b, start_true_anomaly=0.0):
        # left as a placeholder if you need it later
        a = float(a)
        b = float(b)
    ...
    def _append_history_point(self):
        ...
        Uses very lightweight math to minimize overhead.
        """
    ...
    def _acceleration(self, state, t):
        ...
        r_norm = math.sqrt(x*x + y*y)  # faster than np.linalg.norm for 2 elems
        
        if r_norm == 0.0:
            a_grav = np.array([0.0, 0.0])
        else:
            # Use precomputed GM float
...
class DeflectorSatellite:
    def __init__(self, x, y, vx, vy, target_id):
        ...
        # OPTIMIZATION: Store raw data in lists, not dictionaries
        self.hist_t = []
        self.hist_x = []
        self.hist_y = []
        self.hist_vx = []
    ...
    def update(self, dt, target_pos, target_vel, current_sim_time):
        ...
        # ... [Rest of the physics logic remains identical] ...
        ...
        dist = np.linalg.norm(r_vec) # Re-calc dist to aim point
    ...
    # New helper to format data only at the end
...
class InterceptorRocket:
    def __init__(self):
        ...
        # OPTIMIZATION: Changed from list of dicts to separate lists
        self.hist_t = []
        self.hist_x = []
        self.hist_y = []
        self.hist_vx = []
    ...
    try:
        ...
        for sat in defense.satellites:
            sat_data_list.append({
                ...
                # FIXED: This was 'sat.history' which no longer exists. 
                # You must use the helper method.
